...
var Addresstypes = function () {};
Addresstypes.prototype.addresstypes = function (req,res) {
    var params = req.body;
    var query_type = params.query_type;
	console.log("1");
    if(query_type == "fetch_data")
    {
        getAddressTypes(req,res,callBack);
    }else if(query_type == "set_data"){

        req.checkBody('type', 'Please enter address type').notEmpty();
        ...
        if (errors) {  
            ...
            ChcekAddressTypeExists(req,res,callBack);
        }

    }else if(query_type == "del_data") {
		
		DeleteAddressType(req,res,callBack);
	}
	
};


function ChcekAddressTypeExists(req,res,callback){
	var params = req.body;
	var id = params.id;
	var type = params.type;
    //item_category = item_category.trim();
	
	var tbl_name = HMS_HOSP_ADDRESS_TYPES;
...
function InsertAddressType(req,res,data,callback){
	var params = req.body;
	var tbl_name = HMS_HOSP_ADDRESS_TYPES;
    var type = params.type;
...
function UpdateAddressType(req,res,data,callback){
	var params = req.body;
	var tbl_name = HMS_HOSP_ADDRESS_TYPES;
	var id = params.id;
    var type = params.type;
...
//function DeleteItemCategory(req,res,data,callback){
	function DeleteAddressType(req,res,callback){
	var params = req.body;
	var tbl_name = HMS_HOSP_ADDRESS_TYPES;
    var id = params.id;
...
function callBack(err,data,func_type,req,res) {
	if (!isEmptyObject(err) ) {
		...
		if (!isEmptyObject(data)) {
			UpdateAddressType(req,res,data,callBack);
		}else {
			InsertAddressType(req,res,data,callBack);
		}
...
function getAddressTypes(req,res,callback){
    var params = req.body;
    //var user_id = params.user_id;
    var id = params.id;
    
    var tbl1  = HMS_HOSP_ADDRESS_TYPES;
...
exports.Addresstypes = new Addresstypes();