...
var Demographicstypes = function () {};
Demographicstypes.prototype.demographicstypes = function (req,res) {
    var params = req.body;
    var query_type = params.query_type;
	console.log("1");
    if(query_type == "fetch_data")
    {
        getDemographicsTypes(req,res,callBack);
        ...
        if (errors) {  
            ...
            ChcekDemographicsTypeExists(req,res,callBack);
        }

    }else if(query_type == "del_data") {
		
		DeleteDemographicsType(req,res,callBack);
	}
	
};


function ChcekDemographicsTypeExists(req,res,callback){
	var params = req.body;
	var id = params.id;
	var type = params.type;
    //item_category = item_category.trim();
	
	var tbl_name = HMS_HOSP_DEMOGRAPHICS_TYPES;
...
function InsertDemographicsType(req,res,data,callback){
	var params = req.body;
	var tbl_name = HMS_HOSP_DEMOGRAPHICS_TYPES;
    var type = params.type;
...
function UpdateDemographicsType(req,res,data,callback){
	var params = req.body;
	var tbl_name = HMS_HOSP_DEMOGRAPHICS_TYPES;
	var id = params.id;
    var type = params.type;
...
//function DeleteItemCategory(req,res,data,callback){
	function DeleteDemographicsType(req,res,callback){
	var params = req.body;
	var tbl_name = HMS_HOSP_DEMOGRAPHICS_TYPES;
    var id = params.id;
...
function callBack(err,data,func_type,req,res) {
	if (!isEmptyObject(err) ) {
		...
		if (!isEmptyObject(data)) {
			UpdateDemographicsType(req,res,data,callBack);
		}else {
			InsertDemographicsType(req,res,data,callBack);
		}
...
function getDemographicsTypes(req,res,callback){
    var params = req.body;
    //var user_id = params.user_id;
    var id = params.id;
    
    var tbl1  = HMS_HOSP_DEMOGRAPHICS_TYPES;
...
exports.Demographicstypes = new Demographicstypes();